                    ...
                    <form class="layui-form">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="user_code" placeholder="请输入账号" autocomplete="off" class="layui-input">
                            </div>
                            <label class="layui-form-label">姓名</label>
                            <div class="layui-input-inline">
                                <input type="text" name="user_name" placeholder="请输入姓名" autocomplete="off" class="layui-input">
                            </div>
...
function refresh(){
    layui.use('table', function(){
      ...
      table.render({
        elem: '#demo'//绑定容器
        ,url: con.app+'/user/getUserListAll' //数据接口//默认会自动传递两个参数：?page=1&limit=30
        ,height: 'full-230'//容器高度
        ,request: {
          pageName: 'pageIndex' //页码的参数名称，默认：page
          ,limitName: 'pageLimit' //每页数据量的参数名，默认：limit
        }   
        ,where:{user_code:$("input[name='user_code']").val(),user_name:$("input[name='user_name']").val()}
        ,page: true //开启分页
        ,cols: [[ //表头
          ...
          ,{field: 'user_name', title: '姓名', width:120, sort: true}
          ,{field: 'role_code', title: '角色', width:220}
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
      ...
      if(layEvent === 'sel'){ //查看
        ...
        layer.confirm('真的删除行么', function(index){
    	  ...
    	  openLayer('/pro/proupd?id='+data.id,refresh) 
        //同步更新缓存对应的值
//         obj.update({
//           name: '123'
//           ,pass: 'xxx'
